...
lv_obj_t * screen_idle_create(void)
{
    ...
    lv_obj_set_name_static(lv_obj_0, "screen_idle_#");

    lv_obj_add_style(lv_obj_0, &screen_bg, 0);
    lv_obj_t * left_eye = eye_create(lv_obj_0);
    lv_obj_set_name(left_eye, "left_eye");
    lv_obj_set_x(left_eye, 106);
    lv_obj_set_y(left_eye, 156);
    
    lv_obj_t * right_eye = eye_create(lv_obj_0);
    lv_obj_set_name(right_eye, "right_eye");
    lv_obj_set_x(right_eye, 236);
    lv_obj_set_y(right_eye, 156);
    
    
    /* create animation timeline(s) */
    lv_anim_timeline_t ** at_array = lv_malloc(sizeof(lv_anim_timeline_t *) * _SCREEN_IDLE_TIMELINE_CNT);
    at_array[SCREEN_IDLE_TIMELINE_BLINK] = timeline_blink_create(lv_obj_0);
    lv_obj_set_user_data(lv_obj_0, at_array);
    lv_obj_add_event_cb(lv_obj_0, free_timeline_event_cb, LV_EVENT_DELETE, at_array);

    lv_obj_add_play_timeline_event(lv_obj_0, LV_EVENT_SCREEN_LOADED, screen_idle_get_timeline(lv_obj_0, SCREEN_IDLE_TIMELINE_BLINK), 0, false);
...
lv_anim_timeline_t * screen_idle_get_timeline(lv_obj_t * obj, screen_idle_timeline_t timeline_id)
{
    if (timeline_id >= _SCREEN_IDLE_TIMELINE_CNT) {
        LV_LOG_WARN("screen_idle has no timeline with %d ID", timeline_id);
        return NULL;
    }
...
static void free_timeline_event_cb(lv_event_t * e)
{
    ...
    uint32_t i;
    for(i = 0; i < _SCREEN_IDLE_TIMELINE_CNT; i++) {

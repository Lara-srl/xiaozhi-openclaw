...
#include <cstring>
#include "ada_ui_manager.h"  // ← AGGIUNTO
    ...
    board.SetNetworkEventCallback([this](NetworkEvent event, const std::string& data) {
        ...
        switch (event) {
            case NetworkEvent::Scanning:
                ...
                AdaUiManager::GetInstance().SetState(kAdaUiWifiConnecting);  // ← AGGIUNTO
...
void Application::HandleNetworkConnectedEvent() {
    ...
    if (state == kDeviceStateStarting || state == kDeviceStateWifiConfiguring) {
        ...
        AdaUiManager::GetInstance().SetState(kAdaUiActivating);  // ← AGGIUNTO
...
void Application::HandleActivationDoneEvent() {
    ...
    Schedule([this]() {
        ...
        // Auto-connect WS so MCP tools work without button press
        if (protocol_ && !protocol_->IsAudioChannelOpened()) {
            ESP_LOGI(TAG, "Auto-opening audio channel for MCP connectivity");
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsNumber(state_item)) {
                ...
                Schedule([state_code]() {
                    AdaUiManager::GetInstance().SetState(static_cast<AdaUiStateCode>(state_code));
                });
            }
            ...
            if (cJSON_IsString(hex) && strlen(hex->valuestring) == 6) {
                ...
                Schedule([rgb]() { AdaUiManager::GetInstance().SetEyeColor(lv_color_hex(rgb)); });
            }
        } else if (strcmp(type->valuestring, "ping") == 0) {
            // Keepalive from bridge — silently ignore
        } else {
...
void Application::HandleStartListeningEvent() {
    ...
    if (state == kDeviceStateIdle) {
        if (!protocol_->IsAudioChannelOpened()) {
            // Reset AFE ringbuffer before WS handshake to avoid overflow.
            // During OpenAudioChannel() (~1-2s), mic feeds audio into AFE
            // but fetch isn't active yet. Starting with empty buffer avoids overflow.
            audio_service_.ResetProcessor();
            SetDeviceState(kDeviceStateConnecting);
            // Schedule to let the state change be processed first (UI update)
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            audio_service_.EnableWakeWordDetection(true);
            AdaUiManager::GetInstance().SetState(kAdaUiIdle);  // ← AGGIUNTO
            break;
        ...
        case kDeviceStateWifiConfiguring:
            audio_service_.EnableVoiceProcessing(false);
            audio_service_.EnableWakeWordDetection(false);
            AdaUiManager::GetInstance().SetState(kAdaUiWifiConfig);  // ← AGGIUNTO
            break;
        default:
            // Do nothing
            break;
...
bool Application::CanEnterSleepMode() {
    // Block sleep only when actively listening or speaking — not when WS is merely connected.
    // WiFi + WS stay alive during display-sleep (R4: "Display Sleep" strategy).
    auto state = GetDeviceState();
